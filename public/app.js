...
socket.on('nextTurn', id=>{
  ...
  // блокируем кубик если не мой ход или пропуск
  if(id !== socket.id || (me && me.skipNext)){
    btn.disabled = true;
    if(me && me.skipNext){
      showModal("⛔ Вы пропускаете ход");
      me.skipNext = false; // сброс пропуска
      setTimeout(()=>socket.emit('rollDice', roomCode), 1000); // пропуск автоматом
    }
    return;
  }
...
function movePlayerSmooth(id, steps){
  ...
  function step(){
    ...
    // полный круг
    if(p.position === cells.length - 1){
      p.hype += 5;
      showModal("🔥 +5 за круг");
    }

    animateMove(p, p.position, nextIndex, ()=>{
...
function renderLobbyPlayers(){
  ...
  l.innerHTML = players.map(p=>`<div style="color:${p.color}; font-size:20px;">${p.username}</div>`).join("");
}
