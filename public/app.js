...
socket.on('nextTurn', id => {
  ...
  document.getElementById('rollBtn').disabled =
    id !== socket.id || gameOver || isAnimating;
...
function handleCell(p) {
  ...
  switch (cell.type) {
    case 'start': p.hype += 10; text = '🚀 +10 хайпа'; break;
    case 'plus': p.hype += cell.value; text = `➕ +${cell.value}`; break;
    case 'minus': p.hype = Math.max(0, p.hype - cell.value); text = `➖ -${cell.value}`; break;
    case 'skip': p.skipNext = true; text = '🛑 Тюрьма! Пропуск'; break;
    case 'minusSkip': p.hype = Math.max(0, p.hype - cell.value); p.skipNext = true; text = '⚖️ Суд! Пропуск'; break;
